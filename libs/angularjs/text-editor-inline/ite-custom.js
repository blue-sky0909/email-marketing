  ...
  return {
    ...
    link: function($scope, element, attrs, ngModel) {
      ...
      */
      ...
      //$scope.default_fontSize = 12;
     // Generate the font sizes from 7px to 50px
      $scope.get_fontSizes = function () {
        var arr = [];
        for (var i=7; i<=50; i++) {
          // arr.push({name: i+'px', css: i+'px'});
          arr.push(i);
        }
        return arr;
      };
      // console.log($scope.get_fontSizes());

      rangy.init();

      originalToolbar = [ '<div contentEditable="false" name="inlineToolbar" class="btn-group" role="group" aria-label="..." style="z-index:9999">',
                            '<span class="bar-btn bar-btn-dropdown" uib-dropdown on-toggle="toggled(open)">' +
                              '<button uib-dropdown-toggle class="btn btn-default btn-sm" type="button" title="Font">' +
                                '<i class="fa fa-font"></i><i class="fa fa-caret-down"></i>' +
                              '</button>' +
                              '<ul class="uib-dropdown-menu">' +
                                '<li ng-repeat="font in fontFamilies">' +
                                  '<button type="button" ng-click="applyFontFamily(font.family)" data-inline-type="ath-family" class="btn btn-blue checked-dropdown" style="font-family: {{font.family}};">' +
                                   '<i ng-if="font.active" class="fa fa-check"></i>{{font.fontName}}' +
                                  '</button>' +
                                '</li>' +
                              '</ul>' +
                            '</span>',
                            //'<span class="bar-btn"><number-pilot ng-model="fontSize" ng-init="fontSize=default_fontSize" ng-click="$event.preventDefault();" change="applyFontSize()" hide-side-buttons="true" min="1" max="100" step="1" data-inline-type="ath-size" title="Size"></number-pilot></span>',
                            '<span class="bar-btn bar-btn-dropdown" uib-dropdown on-toggle="toggled(open)">'+
                              '<button uib-dropdown-toggle class="btn btn-default btn-sm"  type="button" title="Size">' +
                                '<i class="fa fa-text-height"></i><i class="fa fa-caret-down"></i>' +
                              '</button>' +
                              '<ul class="uib-dropdown-menu">' +
                                  '<li ng-repeat="size in get_fontSizes()">' +
                                    '<button type="button" ng-click="applyFontSize(size)" data-inline-type="ath-size" class="btn btn-blue checked-dropdown">{{size}}</button>' +
                                  '</li>' +
                              '</ul>' +
                            '</span>',
                            ...
                            '<!--<span ng-click="removeToolbar()" class="closeToolbar" title="Close"><i class="fa fa-times"></i></span>-->',
                          '</div>'].join('');
      ...
      element.bind('focus', function (e) {
        $scope.$evalAsync(function() {
          clickPosition = { 'x' : $scope.editor_left, 'y' :$scope.editor_top};
          createToolbar();
          // var range = rangy.getSelection();
          // var start = range.anchorOffset;
          // var end = range.focusOffset;
      ...
      /*
      // Remove toolbar if the user clicks the close icon
      $scope.removeToolbar = function() {
        console.log('clicked');
        removeToolbar('escape');
      };
      */
      ...
      $scope.applyFontFamily = function(family) {
        // this conditional handles the edge case if the user clicks a class button while having link input open
        if (savedSelection && rangy.getSelection().rangeCount == 0) {
        ...
        var opts = {
          ...
          // elementAttributes: {'style': 'font-family:'+family}
        };
        var fontFamilyApplier = rangy.createClassApplier('ath-family', opts);
        // console.log(fontFamilyApplier);

        // angular.element(rangy.getSelection().focusNode).attr('style', 'font-family:'+family);

        // fontFamilyApplier.toggleSelection();
        if(fontFamilyApplier.isAppliedToSelection()) fontFamilyApplier.undoToSelection();
        fontFamilyApplier.applyToSelection();
      ...
      $scope.applyFontSize = function(fontSize) {
        ...
        var opts = {
          ...
          // elementAttributes: {'style': 'font-size:'+fontSize+'px; line-height:100%'}
        };
        var fontSizeApplier = rangy.createClassApplier('ath-size', opts);

        if (fontSizeApplier.isAppliedToSelection()) {
          // removeAttrStyle();
        }

        // fontSizeApplier.toggleSelection();
        if(fontSizeApplier.isAppliedToSelection()) fontSizeApplier.undoToSelection();
        fontSizeApplier.applyToSelection();
      ...
      /*
      // ng-click="clickFontSize($event)"
      $scope.clickFontSize = function(event) {
        event.preventDefault;
        event.stopPropagation;
      };
      */
      ...
      var createToolbar = function(){
        ...
        toolbar[0].style.left = clickPosition.x + 'px';
        toolbar[0].style.top = clickPosition.y - 45 + 'px';
